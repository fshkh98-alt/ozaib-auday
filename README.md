...
- **Rich text / code-block editor** — Blog and Writeup `content` fields are
  currently plain Markdown in a `<textarea>`. Swapping in a proper WYSIWYG
  (e.g. Tiptap) plus a matching MDX renderer on the public pages is a
  self-contained next step now that the data model and forms around it are
  stable.
...
- The three follow-ups above (rich editor, media upload, analytics logging)
