...
export function Sidebar() {
  return (
    <aside className="flex h-screen w-60 flex-col justify-between border-r border-border bg-surface p-4">
      <div>
        <Link href="/admin" className="mb-6 block text-lg font-semibold text-text">
          Ozaib Admin
        </Link>
              ...
              <ul className="space-y-0.5">
                {section.items.map((item) => (
                  <li key={item.href}>
                    <Link
                      href={item.href}
                      className="block rounded-md px-2 py-1.5 text-sm text-text-muted hover:bg-surface-2 hover:text-text"
                    >
                      {item.label}
                    </Link>
                  </li>
